...
Type *parse_raw_type(int *pstorage, Table **pattributes) {
  ...
  for (;;) {
    ...
    case TK_RESTRICT:
      ...
      continue;
    case TK_I8: ++tc.i8_num; continue;
    case TK_I16: ++tc.i16_num; continue;
    case TK_I32: ++tc.i32_num; continue;
    case TK_I64: ++tc.i64_num; continue;
    case TK_U8: ++tc.u8_num; continue;
    case TK_U16: ++tc.u16_num; continue;
    case TK_U32: ++tc.u32_num; continue;
...
loop_exit:;
  ...
  if (type != NULL) {
    ...
    if (tc.f32_num > 0) {
      type = &tyFloat;
    } else if (tc.f64_num > 0) {
      type = &tyDouble;
    } else if (tc.i8_num > 0 || tc.u8_num > 0) {
      type = get_fixnum_type(FX_CHAR, tc.u8_num > 0 || tc.unsigned_num > 0, tc.qualifier);
    } else if (tc.i16_num > 0 || tc.u16_num > 0) {
      type = get_fixnum_type(FX_SHORT, tc.u16_num > 0 || tc.unsigned_num > 0, tc.qualifier);
    } else if (tc.i32_num > 0 || tc.u32_num > 0) {
      type = get_fixnum_type(FX_INT, tc.u32_num > 0 || tc.unsigned_num > 0, tc.qualifier);
    } else if (tc.i64_num > 0 || tc.u64_num > 0) {
      type = get_fixnum_type(FX_LLONG, tc.u64_num > 0 || tc.unsigned_num > 0, tc.qualifier);
    } else {
      type = get_fixnum_type(FX_INT, tc.unsigned_num > 0, tc.qualifier);
    }
  }
