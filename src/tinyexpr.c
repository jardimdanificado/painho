 ...
 *      - TE_VARIABLE enum value and all related handling removed.
 *      - The `bound` field removed from te_expr's union.
 *      - `te_variable` lookup table removed from parser state.
 *      - `find_lookup()` function removed.
 *      - TOK_VARIABLE token removed from the lexer.
 *      - `te_compile()` signature simplified (no variables/var_count args).
 *      - `optimize()` no longer has a TE_VARIABLE early-return guard.
 *      - Unknown identifiers produce a parse error (TOK_ERROR).
 *
 *   2. Comparison operators added (return 1.0 for true, 0.0 for false):
 *        <   >   <=   >=   ==   !=
 *      - Six static helper functions added: cmp_lt, cmp_gt, cmp_le,
 *        cmp_ge, cmp_eq, cmp_ne.
 *      - Lexer updated to recognise single-char (<, >) and two-char
 *        (<=, >=, ==, !=) comparison tokens.
 *      - New grammar rule `comparison()` inserted between `expr()` and
 *        `list()`, giving comparisons lower precedence than +/- and
 *        higher precedence than comma.
 */
        ...
        if ((s->next[0] >= '0' && s->next[0] <= '9') || s->next[0] == '.') {
            ...
            if (isalpha(s->next[0])) {
                ...
                switch (s->next[0]) {
                    ...
                    case '^': s->type = TOK_INFIX; s->function = pow;    s->next++; break;
                    ...
                    case '<':
                        if (s->next[1] == '=') {
                            s->type = TOK_INFIX; s->function = cmp_le; s->next += 2;
                        } else {
                            s->type = TOK_INFIX; s->function = cmp_lt; s->next++;
                        }
                        break;
                    case '>':
                        if (s->next[1] == '=') {
                            s->type = TOK_INFIX; s->function = cmp_ge; s->next += 2;
                        } else {
                            s->type = TOK_INFIX; s->function = cmp_gt; s->next++;
                        }
                        break;
                    case '=':
                        if (s->next[1] == '=') {
                            s->type = TOK_INFIX; s->function = cmp_eq; s->next += 2;
                        } else {
                    ...
                    case '!':
                        if (s->next[1] == '=') {
                            s->type = TOK_INFIX; s->function = cmp_ne; s->next += 2;
                        } else {
                            s->type = TOK_ERROR; s->next++;
                        }
                        break;
...
static te_expr *comparison(state *s) {
    /* <comparison> = <expr> {("<" | ">" | "<=" | ">=" | "==" | "!=") <expr>} */
    te_expr *ret = expr(s);
    CHECK_NULL(ret);

    while (s->type == TOK_INFIX && (
               s->function == cmp_lt || s->function == cmp_gt ||
               s->function == cmp_le || s->function == cmp_ge ||
               s->function == cmp_eq || s->function == cmp_ne)) {
        te_fun2 t = s->function;
        next_token(s);
        te_expr *r = expr(s);
...
static te_expr *list(state *s) {
    ...
    te_expr *ret = comparison(s);
    CHECK_NULL(ret);

    while (s->type == TOK_SEP) {
        next_token(s);
        te_expr *e = comparison(s);
