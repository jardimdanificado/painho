...
"function pap.parse(text)\n"
"    local root = {}\n"
"    local stack = { {node = root, depth = 0} }\n"
"    local lines = {}\n"
"    for line in text:gmatch('([^\\r\\n]*)\\r?\\n?') do\n"
"        if #lines == 0 or line ~= '' or #text > 0 then table.insert(lines, line) end\n"
"    end\n"
"    if #lines > 0 and lines[#lines] == '' then table.remove(lines) end\n"
"    local function flatten(top)\n"
"        local node = top.node\n"
"        local n_cnt, s_cnt, last_n = 0, 0, nil\n"
"        for k, v in pairs(node) do\n"
"            if type(k) == 'number' then n_cnt = n_cnt + 1; last_n = v\n"
"            else s_cnt = s_cnt + 1 end\n"
"        end\n"
"        if n_cnt == 1 and s_cnt == 0 then\n"
"            local parent = stack[#stack].node\n"
"            for pk, pv in pairs(parent) do\n"
"                if pv == node then parent[pk] = last_n; break end\n"
"            end\n"
"        end\n"
"    end\n"
"    local i = 1\n"
...
"                if #stack > 0 then flatten(top) end\n"
"            end\n"
"            local new_node = {}\n"
"            stack[#stack].node[title] = new_node\n"
...
"            while i <= #lines do\n"
"                local tl = lines[i]\n"
"                if not tl:match('^%s*|.*|%s*$') then break end\n"
"                local row, cols = {}, {}\n"
"                for col in tl:gmatch('|([^|]*)') do table.insert(cols, trim(col)) end\n"
"                if #cols > 0 and cols[#cols] == '' then table.remove(cols) end\n"
"                local sep = true\n"
"                for _, cell in ipairs(cols) do\n"
"                    if cell ~= '' and not cell:match('^%-+$') and not cell:match('^:%-+$') then sep = false; break end\n"
"                end\n"
"                if not sep then\n"
"                    local r = {}\n"
"                    for _, c in ipairs(cols) do table.insert(r, tonumber(c) or c) end\n"
"                    table.insert(tbl, r)\n"
"                end\n"
"                i = i + 1\n"
"            end\n"
"            table.insert(stack[#stack].node, tbl)\n"
"        elseif line:match('^%s*```') then\n"
"            local fence = line:match('^%s*(```+)')\n"
"            local code = { line }; i = i + 1\n"
"            while i <= #lines do\n"
"                local cl = lines[i]; table.insert(code, cl); i = i + 1\n"
"                if cl:match('^%s*' .. fence) then break end\n"
"            end\n"
"            table.insert(stack[#stack].node, table.concat(code, '\\n'))\n"
"        elseif line:match('^%s*$') then i = i + 1\n"
"        else\n"
"            local pl = {}\n"
"            while i <= #lines do\n"
"                local l = lines[i]\n"
"                if l:match('^%s*$') or l:match('^(#+)%s*') or l:match('^%s*|.*|%s*$') or l:match('^%s*```') then break end\n"
"                table.insert(pl, l); i = i + 1\n"
"            end\n"
"            local txt = table.concat(pl, '\\n')\n"
"            table.insert(stack[#stack].node, txt)\n"
"        end\n"
"    end\n"
"    while #stack > 1 do\n"
"        local top = table.remove(stack)\n"
...
"    return root\n"
"end\n"
"function pap.generate(node, depth)\n"
"    depth = depth or 0\n"
"    local lines = {}\n"
"    if type(node) == 'string' then return node .. '\\n\\n' end\n"
"    for _, v in ipairs(node) do\n"
"        if type(v) == 'string' then table.insert(lines, v .. '\\n')\n"
"        elseif type(v) == 'table' then\n"
"            if v[1] and type(v) == 'table' and type(v[1]) == 'table' then\n"
"                for ri, row in ipairs(v) do\n"
"                    table.insert(lines, '| ' .. table.concat(row, ' | ') .. ' |')\n"
"                    if ri == 1 then\n"
"                        local s = {}; for _ in ipairs(row) do table.insert(s, '---') end\n"
"                        table.insert(lines, '| ' .. table.concat(s, ' | ') .. ' |')\n"
"                    end\n"
"                end\n"
"                table.insert(lines, '')\n"
"            end\n"
"        end\n"
"    end\n"
"    local keys = {}; for k in pairs(node) do if type(k) == 'string' then table.insert(keys, k) end end\n"
"    table.sort(keys)\n"
"    for _, k in ipairs(keys) do\n"
"        table.insert(lines, string.rep('#', depth + 1) .. ' ' .. k .. '\\n')\n"
"        table.insert(lines, pap.generate(node[k], depth + 1))\n"
"    end\n"
