...
static void math_finalizer(void *userdata) {
    MathState *state = (MathState*)userdata;
    if (state) {
        for (int i = 0; i < state->math_count; i++) {
...
int papagaio_register_math_generic(Papagaio *ctx, const char *name, void *func, int arity, int is_closure, int is_pure, ...) {
    MathState *state = &ctx->math_state;
    void *userdata = NULL;
    if (is_closure) {
        va_list args;
...
Papagaio *papagaio_open(void)
{
    ...
    ctx->math_state.math_funcs = NULL;
    ctx->math_state.math_count = 0;
    ctx->math_state.math_cap = 0;

    papagaio_register_command(ctx, "include", include_handler, NULL);
    papagaio_register_std_commands(ctx);

    /* Register built-in $math{} command */
    papagaio_register_command(ctx, "math", math_cmd_handler, &ctx->math_state);
    papagaio_add_finalizer(ctx, math_finalizer, &ctx->math_state);

    /* Register built-in modifiers */
    papagaio_register_modifier(ctx, "alpha", alpha_modifier, NULL);
    papagaio_register_modifier(ctx, "alphanum", alphanum_modifier, NULL);
