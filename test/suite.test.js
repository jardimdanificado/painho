...
// 8. Compatibilidade com Código Papagaio Legado (via interpretador no novo Papagaio)
import { runLegacy } from "../src/lib/legacy.js";

{
  // Teste 1: Padrão simples
  assert.equal(
    runLegacy("$pattern {$x $y} {$y, $x}\nhello world"),
    "world, hello",
    "Legacy test 1 falhou"
  );

  // Teste 2: Espaçamento flexível
  assert.equal(
    runLegacy("$pattern {$x and $y} {$x & $y}\nhello   and   world"),
    "hello & world",
    "Legacy test 2 falhou"
  );

  // Teste 3: Blocos com delimitadores customizados
  assert.equal(
    runLegacy("$pattern {$content$block{(}{)}} {[$content]}\ndata (hello world)"),
    "data [hello world]",
    "Legacy test 3 falhou"
  );

  // Teste 6: Byte accumulation via repeat
  assert.equal(
    runLegacy("$A$from{}$repeat{3}{$A$byte{65}}$A"),
    "AAA",
    "Legacy test 6 falhou"
  );

  // Teste 7: Repeat list mutation
  assert.equal(
    runLegacy("$L$from{a}$repeat{3}{$L$list{,}$push{x}}$L"),
    "a,x,x,x",
    "Legacy test 7 falhou"
  );

  // Teste 127: Byte básico
  assert.equal(
    runLegacy("$byte{65}$byte{66}$byte{67}"),
    "ABC",
    "Legacy test 127 falhou"
  );

  // Teste 200 e 201: Matemática Louro
  assert.equal(runLegacy("$math{2 + 2}"), "4", "Legacy math addition falhou");
  assert.equal(runLegacy("$math{2 + 3 * 4}"), "14", "Legacy math precedence falhou");
}
